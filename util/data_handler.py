...
class DataHandler(object):
  ...
  def __init__(self, data_pb):
    ...
    data = h5py.File(data_pb.data_file,'r')
    self.handler = data[data_pb.dataset_name][:]
    assert self.handler.shape[0] == sum(self.num_frames_)
    ...
    for v, f in enumerate(self.num_frames_):
      end = start + f - 1 - (self.seq_length_-1)*skip
      ...
      for i in xrange(start, end, self.seq_stride_):
        ...
        lasti = i
      if end - lasti >= self.seq_stride_/2:
        self.video_ind_[end] = v
        labels_.append(init_labels_[v])
  ...
  def GetBatch(self, data_pb, verbose=False):
    ...
    for j in xrange(batch_size):
      ...
      vid_ind[j] = self.video_ind_[start] + 1
      ...
      if length >= (self.seq_length_-1)*skip + 1:
        ...
        self.batch_data_[n:,j,:,:,:] = np.tile(self.batch_data_[n-1,j,:,:,:],(self.seq_length_-n,1,1,1))
      if data_pb.dataset != 'h2mAP':
    ...
    if data_pb.dataset=='ucf11':
      self.batch_data_ = np.transpose(self.batch_data_, (0,4,3,1,2)).astype('float32') 
      #self.batch_data_ = self.batch_data_.reshape([self.batch_data_.shape[0],self.batch_data_.shape[1],1024,49])
      #self.batch_data_ = np.transpose(self.batch_data_, (0,1,3,2)).astype('float32')
    elif data_pb.dataset=='h2mAP':
      self.batch_data_ = self.batch_data_.reshape([self.batch_data_.shape[0],self.batch_data_.shape[1],1024,49]).astype('float32')
    elif data_pb.dataset=='hmdb51gln':
      self.batch_data_ = self.batch_data_.reshape([self.batch_data_.shape[0],self.batch_data_.shape[1],1024,49]).astype('float32')

    self.batch_label_ = self.batch_label_.astype('int64')
    return self.batch_data_, self.batch_label_, n_examples    #self.batch_label_, n_examples, vid_ind
  '''
...
class TrainProto(object):
  def __init__(self, bs, maxlen, stride, dataset, fps=30):
    ...
    self.fps = fps
    if dataset=='ucf11':
      self.data_file       = os.path.join(videoDir, 'train_feats_ucf101.h5')
      self.num_frames_file = os.path.join(videoDir, 'train_nums_ucf101.txt')
      self.labels_file     = os.path.join(videoDir, 'train_labels_ucf101.txt')
...
class TestValidProto(object):
  def __init__(self, bs, maxlen, stride, dataset, fps=30):
    ...
    self.fps = fps
    if dataset=='ucf11':
      self.data_file       = os.path.join(videoDir, 'test_feats_ucf101.h5')
      self.num_frames_file = os.path.join(videoDir, 'test_nums_ucf101.txt')
      self.labels_file     = os.path.join(videoDir, 'test_labels_ucf101.txt')
...
def main():
  fps = 30
  data_pb = TestTrainProto(50,30,10,'ucf11',fps)
