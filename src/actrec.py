...
def init_params(options):
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_state_top', nin=ctx_dim, nout=options['dim'])
    params = get_layer('ff')[0](options, params, prefix='ff_memory_top', nin=ctx_dim, nout=options['dim'])

    params = get_layer('lstm_attend')[0](options, params, prefix='lstm_attend',
                                       nin=options['ctx_dim'], dim=512)    #dim=options['dim']

    # decoder: LSTM - only 1 layer
    params = get_layer('lstm_cond')[0](options, params, prefix='decoder',
                                       nin=options['ctx_dim'], dim=options['dim'])

    params = get_layer('lstm_top')[0](options, params, prefix='decoder_top',
                                       nin=options['dim'], dim=options['dim'])

    params = get_layer('ff')[0](options, params, prefix='ff_fc_out', nin=3*512, nout=options['dim_out'])    #options['dim']
    ...
    if options['n_layers_out'] > 1:
        for lidx in xrange(1, options['n_layers_out']):
            ...
            params = get_layer('ff')[0](options, params, prefix='ff_3c_h%d'%lidx, nin=options['dim_out'], nout=options['dim_out'])
    params = get_layer('ff')[0](options, params, prefix='ff_logit', nin=options['dim_out'], nout=options['n_actions'])
    if options['ctx2out']:
        params = get_layer('ff')[0](options, params, prefix='ff_ctxs', nin=options['dim_out'], nout=options['n_actions']) 
...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'lstm_top': ('param_init_lstm_top', 'lstm_top_layer'),
...
def param_init_lstm_attend(options, params, prefix='lstm_attend', nin=None, dim=None):
    ...
    params[_p(prefix,'W_x_i')] = norm_weight(nin, 1)
    params[_p(prefix,'W_0_i')] = norm_weight(dim, 1)
    params[_p(prefix,'W_1_i')] = norm_weight(dim, 1)
    params[_p(prefix,'W_2_i')] = norm_weight(dim, 1)
    params[_p(prefix,'b_i')] = numpy.zeros((1,)).astype('float32')
...
def lstm_attend_layer(tparams, frame_in, init_c2, prefix='lstm_attend'):
    ...
    def _row_step(x_, c2_, c1_, c0_):
        ...
        c_0 = (1.-i) * c0_ + i * c_0                     # ct = ft * ct-1 + it * at
    ...
    def _row_forward(x_col_, c2_col_, c0_row_0_, c1_col_):
        ...
        return out_c1, out_c2, out_c0, i_col

    c0_row_ = tensor.zeros([7,50,512])

    c1_col_ = tensor.zeros([7,50,512])

    [last_c1, next_c2, last_c0, i_gate], update = theano.scan(_row_forward, sequences=[frame_in, init_c2, c0_row_], 
                                                                             outputs_info=[c1_col_, None, None, None])

    return tensor.concatenate([last_c0[-1][-1], last_c1[-1][-1], next_c2[-1][-1]], axis=-1), next_c2, i_gate
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
    ...
    init_c2 = tensor.zeros([7,7,50,512])

    dim = tparams[_p(prefix, 'U')].shape[0]   # 1024 

    def _step(m_, x_, c2_, h_, c_):
        ...
        c_out, next_c2, i_gate = lstm_attend_layer(tparams, x_, c2_, prefix='lstm_attend')  # current context
        assert c_out.ndim == 2
        c_out = m_[:,None] * c_out
        ...
        for j in xrange(7):
            for i in xrange(7):
                i_gate_list.append(i_gate[j][i])
        alpha = theano.tensor.concatenate(i_gate_list, axis=0)
        alpha = alpha.reshape((n_col*n_row, n_samples)).transpose((1,0))
        alpha_tmp = alpha.norm(2,axis=-1)
        alpha = alpha/(alpha_tmp[:,None]+1e-8)
        x_temp = x_.reshape((n_col*n_row, n_samples, input_dim)).transpose((1,0,2))
        ctx_ = x_temp * alpha[:,:,None]
        ...
        rval = [next_c2, h, c, c_out, alpha, ctx_]
        if options['selector']:
            rval += [sel_]
    ...
    if options['selector']:
        _step0 = lambda m_, x_, c2_, h_, c_, c_ou_, a_, ct_, sel_: _step(m_, x_, c2_, h_, c_, c_ou_, a_, ct_, sel_)
    else:
        _step0 = lambda m_, x_, c2_, h_, c_, c_ou_, a_, ct_: _step(m_, x_, c2_, h_, c_, c_ou_, a_, ct_)


    seqs = [mask, state_below]
    outputs_info = [init_c2, init_state, init_memory, None, None, None]
    if options['selector']:
        outputs_info += [None]
...
def param_init_lstm_top(options, params, prefix='lstm_top', nin=None, dim=None):
    if nin == None:
        nin = options['dim']
    if dim == None:
        dim = options['dim']
...
def lstm_top_layer(tparams, state_below, options, prefix='lstm_top',
                    mask=None, init_memory=None, init_state=None,
                    trng=None, use_noise=None,
                    **kwargs):
    """
    ...
    if options['selector']:
        _step0 = lambda m_, ctx_, h_, c_, sel_: _step(m_, ctx_, h_, c_, sel_)
    else:
        _step0 = lambda m_, ctx_, h_, c_: _step(m_, ctx_, h_, c_)
...
def build_model(tparams, options):
    ...
    # initial state/cell
    ctx_mean = ctx.mean(0).mean(0).mean(0) ### ctx_mean is now (n_samples, ctxdim)
    ...
    init_state_top = get_layer('ff')[1](tparams, ctx_mean, options, prefix='ff_state_top', activ='tanh')
    init_memory_top = get_layer('ff')[1](tparams, ctx_mean, options, prefix='ff_memory_top', activ='tanh')
    
    proj_top = get_layer('lstm_top')[1](tparams, proj_h, options,
                                     prefix='decoder_top',
                                     mask=mask,
                                     init_state=init_state_top,
                                     init_memory=init_memory_top,
                                     trng=trng,
                                     use_noise=use_noise)
    proj_h = proj_top[0]
    ...
    if options['use_dropout']:
        ...
        proj_3c = dropout_layer(proj_3c, use_noise, trng)


    # outputs
    logit = get_layer('ff')[1](tparams, proj_h, options, prefix='ff_logit_lstm', activ='linear')
    logit = tanh(logit)
    proj_3c = get_layer('ff')[1](tparams, proj_3c, options, prefix='ff_fc_out', activ='tanh')
    if options['use_dropout']:
    ...
    if options['n_layers_out'] > 1:
        for lidx in xrange(1, options['n_layers_out']):
            ...
            proj_3c = get_layer('ff')[1](tparams, proj_3c, options, prefix='ff_3c_h%d'%lidx, activ='rectifier')
            if options['use_dropout']:
                logit = dropout_layer(logit, use_noise, trng)
                proj_3c = dropout_layer(proj_3c, use_noise, trng)
    ...
    if options['ctx2out']:
        proj_3c = get_layer('ff')[1](tparams, proj_3c, options, prefix='ff_ctxs', activ='linear')
        proj_3c_shp = proj_3c.shape
        proj_3c = proj_3c.reshape([proj_3c_shp[0]*proj_3c_shp[1], proj_3c_shp[2]])  #(TSxBS,#actions)
        probs_ctx = tensor.nnet.softmax(proj_3c)
        probs_ctx = probs_ctx.reshape([proj_3c_shp[0], proj_3c_shp[1], proj_3c_shp[2]]) #(TS,BS,#actions)
    ...
    if options['ctx2out']:
        probs_ctx = probs_ctx.reshape([proj_3c_shp[0]*proj_3c_shp[1], proj_3c_shp[2]])
        cost_ctx = -tensor.log(probs_ctx[tensor.arange(n_timesteps*n_samples), tmp] + 1e-8)
        cost += cost_ctx
