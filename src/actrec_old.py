...
from util.data_handler import TrainDataHandler
from util.data_handler import ValidDataHandler
from util.data_handler import TestDataHandler
...
def init_params(options):
    ...
    for lidx in xrange(0, options['n_layers_init']):
        ...
        params = get_layer('ff')[0](options, params, prefix='ff_last_state_init_%d'%lidx, nin=ctx_dim, nout=ctx_dim)
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_last_state', nin=options['dim'], nout=2*options['dim'])
    ...
    params = get_layer('lstm_cond')[0](options, params, prefix='decoder',
                                       nin=2*options['dim'], dim=options['dim'],
                                       dimctx=ctx_dim)
...
def param_init_lstm_attend(options, params, prefix='lstm_attend', nin=None, dim=None):
    ...
    W_a = []
    for i in xrange(49):
            W_a.append(norm_weight(2*nin, dim))
    W_a = numpy.array(W_a, dtype=numpy.float32)
    W_a = W_a.reshape(7, 7, 2*nin, dim)
    params[_p(prefix, 'W_a')] = W_a
    params[_p(prefix,'b_i')] = numpy.zeros((dim,)).astype('float32')

    W_x_o = numpy.concatenate([norm_weight(nin,dim),
                               norm_weight(nin,dim)], axis = 1)
    params[_p(prefix,'W_x_o')] = W_x_o

    W_0_o = numpy.concatenate([ortho_weight(dim),
                               ortho_weight(dim)], axis = 1)
    params[_p(prefix,'W_0_o')] = W_0_o

    W_1_o = numpy.concatenate([ortho_weight(dim),
                               ortho_weight(dim)], axis = 1)
    params[_p(prefix,'W_1_o')] = W_1_o
    params[_p(prefix,'b_o')] = numpy.zeros((2*dim,)).astype('float32')

    W_x_c = numpy.concatenate([norm_weight(nin,dim),
                               norm_weight(nin,dim)], axis = 1)
    params[_p(prefix,'W_x_c')] = W_x_c

    W_0_c = numpy.concatenate([ortho_weight(dim),
                               ortho_weight(dim)], axis = 1)
    params[_p(prefix,'W_0_c')] = W_0_c

    W_1_c = numpy.concatenate([ortho_weight(dim),
                               ortho_weight(dim)], axis = 1)
    params[_p(prefix,'W_1_c')] = W_1_c
    params[_p(prefix,'b_c')] = numpy.zeros((2*dim,)).astype('float32')

    return params


def lstm_attend_layer(tparams, frame_in, last_state, prefix='lstm_attend'):
    # frame_in.shape: [n_col, n_row, batch_size, input_dim]
    assert frame_in.ndim == 4
   
    dim = tparams[_p(prefix, 'W_x_i')].shape[-1]  

    def _row_step(x_, Wa_, h1_, c1_, h0_, c0_):
        assert x_.ndim == 2
        assert Wa_.ndim == 2
        i_gate = tensor.dot(x_, tparams[_p(prefix, 'W_x_i')]) + tensor.dot(h0_, tparams[_p(prefix, 'W_0_i')]) + tensor.dot(h1_, tparams[_p(prefix, 'W_1_i')]) + \
                tensor.dot(last_state, Wa_) + tparams[_p(prefix, 'b_i')]
        i_gate = tensor.nnet.sigmoid(i_gate)
        
        o_gate = tensor.dot(x_, tparams[_p(prefix, 'W_x_o')]) + tensor.dot(h0_, tparams[_p(prefix, 'W_0_o')]) + tensor.dot(h1_, tparams[_p(prefix, 'W_1_o')]) + \
                 tparams[_p(prefix, 'b_o')]
        o_0 = _slice(o_gate, 0, dim)
        o_0 = tensor.nnet.sigmoid(o_0)
        c_pre = tensor.dot(x_, tparams[_p(prefix, 'W_x_c')]) + tensor.dot(h0_, tparams[_p(prefix, 'W_0_c')]) + tensor.dot(h1_, tparams[_p(prefix, 'W_1_c')]) + \
                tparams[_p(prefix, 'b_c')]
        c_0 = _slice(c_pre, 0, dim)
        c_0 = (1-i_gate) * c0_ + i_gate * c_0
        h0 = o_0 * tensor.tanh(c_0)

        o_1 = _slice(o_gate, 1, dim)
        o_1 = tensor.nnet.sigmoid(o_1)
        c_1 = _slice(c_pre, 1, dim)
        c_1 = (1-i_gate) * c1_ + i_gate * c_1
        h1 = o_1 * tensor.tanh(c_1)

        return h0, c_0, h1, c_1
    
    def _row_forward(x_col_, Wa_col_, h0_row_0_, c0_row_0_, h1_col_, c1_col_):
        assert x_col_.ndim == 3
        assert Wa_col_.ndim == 3
        [out_h0, out_c0, out_h1, out_c1], update = theano.scan(_row_step, sequences=[x_col_, Wa_col_, h1_col_, c1_col_], 
                                                                          outputs_info=[h0_row_0_, c0_row_0_, None, None])
        
        return out_h1, out_c1, out_h0[-1], out_c0[-1]

    h0_row_ = tensor.zeros_like(frame_in[:,0,:,:])
    c0_row_ = tensor.zeros_like(frame_in[:,0,:,:])

    h1_col_ = tensor.zeros_like(frame_in[0,:,:,:])
    c1_col_ = tensor.zeros_like(frame_in[0,:,:,:])

    [last_h1, last_c1, last_h0, last_c0], update = theano.scan(_row_forward, sequences=[frame_in, tparams[_p(prefix, 'W_a')], h0_row_, c0_row_], 
                                                                             outputs_info=[h1_col_, c1_col_, None, None])

    return tensor.concatenate([last_h0[-1], last_h1[-1][-1]], axis=-1)
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
                    mask=None, init_state=None, init_memory=None, init_last_state=None,
    ...
    if init_last_state ==None:
        init_last_state = tensor.alloc(0., n_samples, 2*dim)
    ...
    def _step(m_, x_, h_, c_, ct_, dp_=None, dp_att_=None):
        ...
        ctx_ = lstm_attend_layer(tparams, x_, ct_, prefix='lstm_attend')  # current context
        assert ctx_.ndim == 2
        # print '\n\ncheck\n\n'
        if options['selector']:
...
def build_model(tparams, options):
    ...
    # initial state/cell
    ctx_mean = ctx.mean(0)
    ctx_mean = ctx_mean.mean(0) 
    ctx_mean = ctx_mean.mean(0)  ### you want ctx_mean to be n_samples x ctxdim

    ctx_mean_init = get_layer('ff')[1](tparams, ctx_mean, options,
                                      prefix='ff_init_0', activ='rectifier')
    ctx_mean_laststate = get_layer('ff')[1](tparams, ctx_mean, options,
                                      prefix='ff_last_state_init_0', activ='rectifier')
    if options['use_dropout']:
        ctx_mean_init = dropout_layer(ctx_mean_init, use_noise, trng)
        ctx_mean_laststate = dropout_layer(ctx_mean_laststate, use_noise, trng)

    if options['n_layers_init'] > 1:
        for lidx in xrange(1, options['n_layers_init']):
            ctx_mean_init = get_layer('ff')[1](tparams, ctx_mean_init, options,
                                      prefix='ff_init_%d'%lidx, activ='rectifier')

            ctx_mean_laststate = get_layer('ff')[1](tparams, ctx_mean_laststate, options,
                                      prefix='ff_last_state_init_%d'%lidx, activ='rectifier')
            if options['use_dropout']:
                ctx_mean_init = dropout_layer(ctx_mean_init, use_noise, trng)
                ctx_mean_laststate = dropout_layer(ctx_mean_laststate, use_noise, trng)

    init_state = get_layer('ff')[1](tparams, ctx_mean_init, options, prefix='ff_state', activ='tanh')
    init_memory = get_layer('ff')[1](tparams, ctx_mean_init, options, prefix='ff_memory', activ='tanh')
    init_last_state = get_layer('ff')[1](tparams, ctx_mean_laststate, options, prefix='ff_last_state', activ='tanh')
    ...
    proj = get_layer('lstm_cond')[1](tparams, ctx, options,
                                     ...
                                     init_last_state=init_last_state,
                                     trng=trng,
                                     use_noise=use_noise)
    # collection
    proj_h = proj[0]
    ...
    return trng, use_noise, [x, mask, y], cost, opt_outs, preds
...
def train(dim_out=500, # hidden layer dim for outputs
    ...
    dh_test_train = TrainDataHandler(data_test_train_pb)
    ...
    dh_test_valid = ValidDataHandler(data_test_valid_pb)
    ...
    dh_test_test = TestDataHandler(data_test_test_pb)
